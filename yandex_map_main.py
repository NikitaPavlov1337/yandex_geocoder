...
def readFile(fileName):
    ...
    words = fileObj.read().splitlines()[9000:9999] # puts the file into an array
    fileObj.close()
    return words
...
stopwords = ['нежилое','помещение','торговый','нежилого', 'территории','нежилых','в части','часть','пом',
             ...
             'номера на поэтажном плане','(нежилые помещения','расположенные','-аптека','на углу','торговый павильон']
result = []
for str in file:
    str = str.lower()
    list = str.split(', ')
...
file2 = open('out/new_output11.csv', 'w+', newline='')
with file2:
